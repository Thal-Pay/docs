...
curl -X GET "https://api.thalpay.com/transactions?take=20&skip=0&status=PAID&method=PIX" \
  -H "x-api-key: sua_api_key"
```

```bash Filtro por metadata
curl -X GET "https://api.thalpay.com/transactions?metadataKey=orderId&metadataValue=ORD-001" \
  -H "x-api-key: sua_api_key"
```

```bash Exportar CSV
curl -X GET "https://api.thalpay.com/transactions?export=true&status=PAID" \
  -H "x-api-key: sua_api_key" \
  -o transacoes.csv
```
